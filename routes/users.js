...
//if user want to change password from inside
...
router.post('/forgot', (req, res, next)=> {
    ...
    async.waterfall([
        ...
        (token, user) => {
            ...
            let mailOptions = {
                to: user.email,
                from : 'Prashant Raj praaj993@gmail.com',
...
router.post('/reset/:token', (req, res)=>{
    async.waterfall([ //runs array of function 1 afther another asyncly
        (done) => {
            User.findOne({resetPasswordToken: req.params.token, resetPasswordExpires : {$gt : Date.now() } })
                .then(user => {
                    if(!user) {
        ...
        (user) => {
            let smtpTransport = nodemailer.createTransport({ //smtp: simple mail transfer protocol
                service : 'Gmail',
                auth:{
                    user : process.env.GMAIL_EMAIL,
                    pass : process.env.GMAIL_PASSWORD
